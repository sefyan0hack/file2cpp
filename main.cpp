...
int main(int argc, char* argv[]){
    ...
    static auto Pack_File = fstream(fs::path(out_dir) / Pack, ios::out | ios::trunc);

    Pack_File << "#include <string>" << endl;
    Pack_File << "#include <unordered_map>" << endl;
    Pack_File << "#include <utility>" << endl;
    ...
    | views::filter([](const auto& e) { return e.is_regular_file(); }))
    {
        ...
        Pack_File << file2code(filename) << endl;
    }

    //
    Pack_File << "std::unordered_map<std::string, std::pair<const unsigned char*, size_t>> Resources {" << endl;
    ...
    | views::filter([](const auto& e) { return e.is_regular_file(); }))
    {
        ...
        Pack_File << '\t' << format(R"({{ "{}", {{ res_{}, {} }} }},)", filename, hash<string>{}(filename), size) << endl;
    }
